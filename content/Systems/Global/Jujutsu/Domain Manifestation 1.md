...
This technique that allows the user to shape cursed energy to complete or form a blade or tool-head. The cursed energy blade has physical properties akin to a real sword. 
- This technique costs an additional 10 Cursed Energy on top of the cost to infuse your weapon with Cursed Energy.
- The strength and durability of the tool-head their C.E.O + their infusion buff.
...
[ 40 C.E.O / 10 Skill Points ]
_Domain Amplification_ (領域展延 lit. _Domain Envelopment_) _is an anti-domain technique that envelops the user in a domain to neutralise any opposing cursed technique or domain expansion they come into contact with._

### Description
The user wraps themselves thinly in a domain unimbued with a cursed technique, allowing them to pour an opponent's technique into the empty space and negate it on contact. This means an attack with amplification will always land regardless of the opponent's technique. It will negate any technique it encounters and weaken the sure-hit effect imbued into opposing domains as well

For example, an amplification user can neutralise the Infinity by allowing the [[Limitless]] technique to flow into the domain on contact. While similar to simple domain, amplification is more refined. It has the potential to negate an opposing technique and the user can move freely rather than having to remain rooted in place.
